...
#define DATALOG  1                //if uncommented : data log to serial actived
bool USAGE_FPRI = true;           // if true : the turbine speed is calculated

// data log to serial interval millis
unsigned long previousMillis = 0; //variabel untuk menyimpan waktu sebelumnya
unsigned long interval = 100; //interval waktu dalam milidetik
...
float VsorMin = 8;   //aslinya 2.5x3    // discharged battery voltage : switch off the inverter (see battery datasheet for exact value)
float VsorFlo = 10.92;       // floating voltage (acid) or nominal voltage (lithium) (see battery datasheet for exact value)
float VsorMax = 12.6;       // maximum voltage (see battery datasheet for exact value)
int   IbatMax = 4;          // acid lead batteries : must be = 0,23 time the battery capacity => 13A for 54Ah batteries
...
int VpriMin = 0;           // the voltage that will start the MPPT process. Too low the wind turbine may have difficulties to start
...
void setup() {
  ...
  pinMode(loadPin,   OUTPUT); digitalWrite(loadPin, HIGH);    // output for dumpload resistor
  pinMode(ondulPin,  OUTPUT); digitalWrite(ondulPin, HIGH);   // output for enabling injection
  ...
  // Console logging serial initialization
#ifdef DATALOG
  Serial.begin(115200);
  Serial.println();
  Serial.println(F("Ready to start logging..."));
#endif
...
void loop() {
  ...
  if ((lect_Ipri < 2) || (lect_Ipri > 1021) || (lect_Ibat < 2) || (lect_Ibat > 1021)) {
    ...
    digitalWrite(ondulPin, HIGH);             // swith off inverter
  ...
  if ((USAGE_FPRI && (Fpri_flag || ((tempo - memo_tempo) > 100)))
    ...
    if ( Vsor < VsorMin ) {
      digitalWrite(ondulPin, HIGH);     // stop the inverter
    ...
    if (( Vsor > VsorFlo ) && okInverter ) digitalWrite(ondulPin, LOW); // start the inverter
    ...
    if ( Vsor > VsorMax ) {                   // when VsorMax is reached
      ...
      digitalWrite(loadPin, LOW);            // dumpload is ON
    }
    else {
    ...
    // for logging serial purpose only
    unsigned long currentMillis = millis(); //mengambil waktu saat ini
    if (currentMillis - previousMillis >= interval) { //jika interval waktu telah tercapai
      previousMillis = currentMillis; //simpan waktu sekarang sebagai waktu sebelumnya
#ifdef DATALOG
      Serial.print("[" + String(Fpri) + "," + String(Vpri) + "," + String(Ipri) + "," + String(Puiss) + "," + String(Step) + "," + String(pwm_gate) + "," + String(Vsor) + "," + String(Ibat) + "]");
      Serial.println();
#endif
    }
    ...
    if ( window < 2 ) {
      ...
      if (whichLine) {
        ...
        if ( window == 0 ) {
          ...
          dtostrf(Puiss, 4, 2, flt2str);
          strcat(lcdRow, flt2str);

        }
        else if ( window == 1 ) {
          dtostrf(Ipri, 4, 2, flt2str);
          strcpy(lcdRow, "Ie="); strcat(lcdRow, flt2str);
          dtostrf(Ibat, 4, 2, flt2str);
      ...
      if ( window == 6 ) {
        ...
        VpriMin = constrain(VpriMin, 1, 80);
      ...
      if ( window == 11 ) {
        ...
        if (ret_push_button == 3) Vpri_calibrate -= 0.01;
      ...
      if ( window == 12 ) {
        ...
        if (ret_push_button == 3) Vsor_calibrate -= 0.01;
